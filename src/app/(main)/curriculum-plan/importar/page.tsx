...
const PLANNING_TYPE_VALUES: CurriculumPlanningType[] = ["annual", "semester", "trimester", "custom"];
...
type StepId = "upload" | "import" | "normalize" | "done";
...
const STEP_IDS: StepId[] = ["upload", "import", "normalize", "done"];
...
export default function CurriculumPlanImportPage() {
  const t = useTranslations("curriculumPlan.importar");
  ...
  const stepLabels: Record<StepId, string> = {
    ...
    normalize: t("steps.normalize"),
    done: t("steps.done"),
  };
  const INITIAL_STEPS: ImportStep[] = STEP_IDS.map((id) => ({
    id,
    label: stepLabels[id],
    status: "pending",
  }));
        ...
        <Card>
          <CardContent className="flex flex-col items-center gap-8 py-12">
            <div className="text-center">
              <p className="text-base font-medium">{t("importing.title")}</p>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                <Label htmlFor="schoolYear">{t("form.schoolYearLabel")}</Label>
                <Select
            ...
            <div className="space-y-2">
              <Label htmlFor="planningType">{t("form.planningTypeLabel")}</Label>
              <Select
                value={planningType}
              ...
              >
                ...
                <SelectContent>
                  {PLANNING_TYPE_VALUES.map((value) => (
                    <SelectItem key={value} value={value}>
                      {tShared(`planningType.${value}`)}
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="space-y-2">
                <Label htmlFor="periodStart">{t("form.periodStartLabel")}</Label>
                <Input
                  id="periodStart"
                  type="date"
                  value={periodStart}
              ...
              <div className="space-y-2">
                <Label htmlFor="periodEnd">{t("form.periodEndLabel")}</Label>
                <Input
                  id="periodEnd"
                  type="date"
                  value={periodEnd}
